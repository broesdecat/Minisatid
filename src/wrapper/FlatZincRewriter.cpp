...
void FlatZincRewriter::check(const Literal& lit){
	...
	bool add = false;
	if(seenset.find(value)==seenset.end()){
...
const Weight& FlatZincRewriter::getMin(uint var){
	assert(varbounds.find(var)!=varbounds.end());
...
void addMappedList(const vector<T>& list, ostream& stream){
	bool begin = true;
	for(typename vector<T>::const_iterator i=list.begin(); i<list.end(); ++i){
		if(!begin){
			stream <<", ";
		}
		begin = false;
...
void FlatZincRewriter::addSum(const weightlist& weights, const vector<uint>& vars, const Atom& head, EqType rel, const Weight& bound){
	stringstream ss;
	bool begin = true;
	for(vector<uint>::const_iterator i=vars.begin(); i<vars.end(); ++i){
		if(!begin){
			ss <<", ";
		}
		begin = false;
...
uint FlatZincRewriter::addOptimization(){
	...
	if(optim==MNMZ_AGG){
		...
		Weight min = 0, max = 0;
		for(weightlist::const_iterator i=set.weights.begin(); i<set.weights.end(); ++i){
			if((*i)<0){
				min += *i;
			}else{
				max += *i;
		...
		int currentvalue = 1;
		for(literallist::const_iterator i=mnm.literals.begin(); i<mnm.literals.end(); ++i){
			stringstream ss;
			ss <<currentvalue;
...
void FlatZincRewriter::finishParsing(){
	...
	for(vector<BinRel>::const_iterator i=savedbinrels.begin(); i<savedbinrels.end(); ++i){
		addBinRel((*i).left, (*i).right, Literal((*i).head, false), (*i).rel);
	}

	for(vector<CPSumWeighted>::const_iterator i=savedcpsums.begin(); i<savedcpsums.end(); ++i){
		addSum((*i).weights, (*i).varIDs, (*i).head, (*i).rel, (*i).bound);
	}

	for(vector<Aggregate>::const_iterator i=savedaggs.begin(); i<savedaggs.end(); ++i){
		if((*i).type==PROD){
...
void FlatZincRewriter::addIntegerVar(uint varID, const string& domainexpr, const Weight& min, const Weight& max){
	if(cpvarsseen.find(varID)!=cpvarsseen.end()){
		stringstream ss;
...
template<>
bool FlatZincRewriter::add(const MinimizeAgg& mnm){
	assert(isParsing());
	check(mnm.head);
	savedaggmnmz = mnm;
	optim = MNMZ_AGG;
...
bool FlatZincRewriter::add(const CPIntVarEnum& var){
	...
	ss <<"{";
	bool begin = true;
	for(vector<Weight>::const_iterator i=var.values.begin(); i<var.values.end(); ++i){
		if(!begin){
			ss <<", ";
		}
		begin = false;
