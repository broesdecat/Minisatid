...
void ModSolver::finishParsing(){
	//TODO keep such information in extra struct per modal solver/sat-solver
	bool aggpresent = false, defpresent = false;
	if(aggsolver.get()!=NULL){
		aggpresent = aggsolver->finishECNF_DataStructures();
	}
	if(idsolver.get()!=NULL){
		defpresent = idsolver->finishECNF_DataStructures();
	}
	...
	for(vmodindex::const_iterator i=getChildren().begin(); i<getChildren().end(); i++){
		modhier.lock()->getModSolver(*i)->finishParsing();
	}
}
