...
bool PbSolver::normalizePb(vec<Lit>& ps, vec<Int>& Cs, Int& C, bool skipTriv) {
	...
	for (int i = 0; i < Csps.size(); i++) {
		...
		if (sum < 0)
			fprintf(stderr, "ERROR! Too large constants encountered in constraint.\n"), exit(1);
	}
...
bool PbSolver::validateResoult() {
	if (opt_verbosity >= 1)
		std::cout << "starting validation number of cons=" << constrs.size() << "\n";
	for (int i = 0; i < constrs.size(); i++) {
		...
		for (int i = 0; i < c.size; i++) {
			if ((sign(c[i]) && !best_model[var(c[i])]) || (!sign(c[i]) && best_model[var(c[i])]))
				sum += c(i);
		}
		if (opt_verbosity >= 1) {
	...
	if (opt_verbosity >= 1)
		std::cout << "end validation all is good!" << "\n";
	return true;
}
...
void PbSolver::propagate() {
	...
	if (opt_verbosity >= 1)
		reportf("  -- Unit propagations: ", constrs.size());
	bool found = false;
...
void PbSolver::setupOccurs() {
	...
	for (int i = 0; i < constrs.size(); i++) {
		...
		for (int j = 0; j < constrs[i]->size; j++)
			assert(occur[index((*constrs[i])[j])].size() < n_occurs[index((*constrs[i])[j])]), occur[index((*constrs[i])[j])].push(i);
	}
}
...
static Int evalGoal(Linear& goal, vec<lbool>& model) {
	Int sum = 0;
	for (int i = 0; i < goal.size; i++) {
		assert(model[var(goal[i])] != l_Undef);
		if ((sign(goal[i]) && model[var(goal[i])] == l_False) || (!sign(goal[i]) && model[var(goal[i])] == l_True))
			sum += goal(i);
	}
	return sum;
}
...
bool PbSolver::toCNF(std::vector<std::vector<Lit> >& cnf) {
	...
	if (opt_polarity_sug != 0) {
		for (int i = 0; i < goal_Cs.size(); i++)
			sat_solver.suggestPolarity(var(goal_ps[i]), ((goal_Cs[i] * opt_polarity_sug > 0 && !sign(goal_ps[i])) || (goal_Cs[i] * opt_polarity_sug
					< 0 && sign(goal_ps[i]))) ? l_False : l_True);
	}
	...
	if (opt_goal != Int_MAX)
		addConstr(goal_ps, goal_Cs, opt_goal, -1, false), convertPbs(false);
...
void PbSolver::solve(solve_Command cmd, bool skipSolving) {
	...
	if (opt_cnf != NULL)
		reportf("Exporting CNF to: \b%s\b\n", opt_cnf), sat_solver.exportCnf(opt_cnf), exit(0);
	...
	while (sat_solver.solve()) {
		...
		if (cmd == sc_AllSolutions) {
			...
			best_model.clear();
			for (Var x = 0; x < pb_n_vars; x++)
				assert(sat_solver.model()[x] != l_Undef), best_model.push(sat_solver.model()[x] == l_True);
