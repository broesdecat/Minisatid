...
inline uint64 nlg2n2(register uint64 v) {
  register uint64 lg2 = (uint64)(llrint(log2(v)));
  if (lg2==0) lg2=1;
  return lg2*lg2*v;
}


inline uint64 oddEvenCount(register uint64 v) {
		         /*if      (v<=1) return   0;
