...
l1: childCarryins += childCost/msb;
	switch(childLastRelevent) {
		case 0 : 
				childCost=oddEvenCount(childCost)+father->cost+oddEvenCount(childCarryins);
		...
		default:
				...
				if (!abstraction){
					...
					if (temp) {
					     if (childCost+oddEvenCount(childCarryins) < temp->cost+oddEvenCount(temp->carryins)){
					     	temp->parent = father;
					    	temp->cost  = childCost;
					    	temp->hCost = h; 
...
int mainOE(int argc, char **argv) {
	...
	SearchMetaData* md = bnb_oddEven_Cost_search(ws,length(ws),pri,1000000,false,true);
	md->print();
	delete md;
	return 0; 
}
