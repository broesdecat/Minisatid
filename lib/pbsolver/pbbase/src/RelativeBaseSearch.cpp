...
static inline void createChild(bssr father,register unsigned int msb,register unsigned int wights[][2],unsigned int sum[],uint64& cnfSizeOfBestFound,uint64& carryOfBestFound,
...
static inline uint64 calcCarryCost(std::vector<uint64> &carry) {
	uint64 cuerntChainLegth = 0;
	uint64 carryCost = 0;
...
SearchMetaData* bnb_Relative_search(unsigned int weights[][2],int length,std::vector<unsigned int>& primes,unsigned int cutoff,bool nonPrimePossible,bool abstraction){
		...
		THeapComp que;
		uint64 carryOfBestFound,cnfSizeOfBestFound;
...
//			std::cout<<","<<carry[tt];
	//	}
		...
		//carry.clear();
		//carryVecEval(weights, md->base,length,carry);
		//uint64 byFunc = calcCarryCost(carry);
		//for(unsigned int tt=0;tt<carry.size();tt++){
	//		std::cout<<","<<carry[tt];
	//	}
		//std::cout<<"\n";
...
l1:	uint64 ttt = childCost/msb;
 	childCarryins += ttt;
	int curC;
	uint64 cc = father->carryCost;
	if (ttt==0)
		curC = 0;
	else 
