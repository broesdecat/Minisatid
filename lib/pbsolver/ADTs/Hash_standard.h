...
#ifdef LP64
template <class K> struct Hash<const K*> { unsigned int operator () (const K* key) const { unsigned intp tmp = reinterpret_cast<unsigned intp>(key); return (unsigned)((tmp >> 32) ^ tmp); } };
