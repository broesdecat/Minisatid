...
template<class B>
static void parse_ECNF_main(B& in, Solver* S, IDSolver* TS, AggSolver* AGG/*, AMNSolver* AS*/) { // NOTE: this parser does not read translation information.
	vec<Lit> lits;
	for (;;) {
		skipWhitespace(in);
		char c = *in;
...
template<class B>
static void parse_main(B& in, Solver* S, IDSolver* TS, AggSolver* AGG/*, AMNSolver* AS*/) {
	bool ecnf = false;
	for (;;) {
		skipWhitespace(in);
		if (*in == EOF)
...
//
static void parse(gzFile input_stream, Solver* S, IDSolver* TS, AggSolver* AGG/*, AMNSolver* AS*/) {
